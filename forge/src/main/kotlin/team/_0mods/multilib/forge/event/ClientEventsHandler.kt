...
class ClientEventsHandler {
    ...
    fun tooltip(e: ItemTooltipEvent) {
        TooltipEvent.ITEM.event.append(e.itemStack, e.flags, e.toolTip)
    }
    ...
    fun postRender(e: RenderGuiEvent.Post) {
        ScreenEvent.RENDER_HUD.event.render(e.guiGraphics, e.partialTick)
    }
    ...
    fun input(e: ForgeInput.MouseButton.Post) {
        InputEvent.MOUSE_CLICK_POST.event.onClicked(Minecraft.getInstance(), e.button, e.action, e.modifiers)
    }

    @SubscribeEvent(priority = EventPriority.HIGH)
    fun input(e: ForgeInput.Key) {
        InputEvent.KEY_PRESS.event.onPress(Minecraft.getInstance(), e.key, e.scanCode, e.action, e.modifiers)
    }
