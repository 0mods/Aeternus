...
fun configScreen() = ImGuiScreen {
    ...
    val experimentalCategory = AeternusCommonConfig.CategoryExperimental(
        CommentedValue.create(booleanValue("gui.aeternus.config.experimental_features", commonConfig.debug()), *defaultConfig.debug.comment.toTypedArray())
    )

    regenerateCfg(
        AeternusCommonConfig(
            CommentedValue.create(booleanValue("gui.aeternus.config.debug", commonConfig.debug()), *defaultConfig.debug.comment.toTypedArray()),
