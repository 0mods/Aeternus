...
import team._0mods.aeternus.api.util.exists
...
object GltfTree {
    ...
    fun parse(resource: ResourceLocation): GLTFTree {
        ...
        val file = if (location.path.endsWith(".glb")) {
            val bytes = location.stream.readBytes()
            ...
            GltfDefinition.parse(ByteArrayInputStream(data)).apply {
                ...
                while (dataStream.available() > 0) {
                    ...
                    if (chunkType == GltfFile.GLB_CHUNK_MAGIC_BIN) {
                        ...
                        LOGGER.warn("Unexpected chunk type for bin chunk: $chunkType (should be ${GltfFile.GLB_CHUNK_MAGIC_BIN}: BIN)")
                        dataStream.skip(chunkLength.toLong())
                    }
                }
                this.binaryBuffer = array
            }
        } else GltfDefinition.parse(location.stream)
    ...
    private fun getMaterial(
        ...
        if (textureId == -1) return if (material.name != null) Material(
            color,
            "textures/models/${material.name}.png".lowercase().aRl,
    ...
    fun getTexture(
        ...
        file.images[image].bufferView?.let { bufferId ->
            ...
            val textureLocation = textureName.lowercase().aRl
...
val hasFirstPersonModel = PlatformHelper.isModLoaded("firstperson")
