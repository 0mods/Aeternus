...
class GifTexture(location: ResourceLocation) : SimpleTexture(location) {
    ...
    override fun getId(): Int {
        val id = super.getId()
        val time = (TickHandler.currentTicks + Minecraft.getInstance().deltaFrameTime) % fullTime
    ...
    override fun load(pResourceManager: ResourceManager) {
        try {
            ...
            LOGGER.warn("Error while loading the texture", var5)
        }
    }
        ...
        if (!imageReaders.hasNext()) {
            ...
            if (reader.formatName.equals("gif", ignoreCase = true)) {
                ...
                try {
                    ...
                    LOGGER.error("Failed to parse input format", var9)
                } finally {
                    reader.dispose()
                    stream.close()
                }
